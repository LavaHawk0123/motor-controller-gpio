...
{
    right_vel = abs(linear_vel - (angular_vel * wheel_base * 3)) * 2;
    left_vel = abs(linear_vel + (angular_vel * wheel_base * 3)) * 2;
    setRPM();
    setDirection();
}
...
void motor_controller::settingDutyCycle()
{
    ...
    if (right_dutyCyclePercentage > 255)
        right_dutyCyclePercentage = 254;
    if (left_dutyCyclePercentage > 255)
        left_dutyCyclePercentage = 254;
}
    ...
    while (ros::ok())
    {
        ...
        if (motor_control.pwm_start == 1)
        {
            if (motor_control.direction == 1)
            {
                ...
                std::cout << "Left Duty Cycle: " << motor_control.left_dutyCyclePercentage / 2.55 << "% ";
                gpioPWM(13, motor_control.right_dutyCyclePercentage);
                std::cout << "Right Duty Cycle: " << motor_control.right_dutyCyclePercentage / 2.55 << "% ";
            ...
            {
                gpioPWM(12, motor_control.left_dutyCyclePercentage * 2);
                std::cout << "Left Duty Cycle: " << motor_control.left_dutyCyclePercentage * 2 / 2.55 << "% ";
                gpioPWM(13, motor_control.right_dutyCyclePercentage * 2);
                std::cout << "Right Duty Cycle: " << motor_control.right_dutyCyclePercentage * 2 / 2.55 << "% ";
                std::cout << "Direction: " << motor_control.direction << std::endl;
                gpioWrite(2, 1);
                gpioWrite(3, 0);
                gpioWrite(0, 1);
